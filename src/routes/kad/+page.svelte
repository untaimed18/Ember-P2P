<script lang="ts">
  ...
  let bootstrapMode: BootstrapMode = $state('ip');
  ...
  type BootstrapMode = (typeof bootstrapModes)[number];

  function bootstrapModeLabel(mode: BootstrapMode): string {
    switch (mode) {
      case 'ip': return m.kad_bootstrap_tab_ip();
      case 'url': return m.kad_bootstrap_tab_url();
      case 'clients': return m.kad_bootstrap_tab_clients();
    }
  }

  /**
   * Arrow/Home/End movement across the bootstrap tabs, per the ARIA tabs
   * pattern. Selection follows focus, which is right for three cheap panels
   * that fetch nothing on switch.
   *
   * Bound to each tab rather than the tablist: only the selected tab is in the
   * tab order (roving tabindex), so the container is never the thing holding
   * focus and has no business listening for keys.
   */
  function bootstrapTabsKeydown(e: KeyboardEvent) {
    const keys = ['ArrowLeft', 'ArrowRight', 'Home', 'End'];
    if (!keys.includes(e.key)) return;
    e.preventDefault();
    const current = bootstrapModes.indexOf(bootstrapMode);
    const next =
      e.key === 'Home'
        ? 0
        : e.key === 'End'
          ? bootstrapModes.length - 1
          : (current + (e.key === 'ArrowRight' ? 1 : -1) + bootstrapModes.length) %
            bootstrapModes.length;
    bootstrapMode = bootstrapModes[next];
    document.getElementById(`kad-bs-tab-${bootstrapModes[next]}`)?.focus();
  }
...
<div class="page-header">
  <!-- Subtitle, like the Ember page: this is a table of DHT peers, and it used
       to be the first screen after setup with nothing on it saying what KAD is
       or why the user is looking at it. -->
  <div>
            ...
            <thead>
              <tr>
                ...
                <th
                  ...
                  tabindex="0"
                  role="columnheader"
                  aria-sort={contactSortCol === 'distance' ? (contactSortAsc ? 'ascending' : 'descending') : 'none'}
                  onclick={() => sortContacts('distance')}
                  onkeydown={(e) => (e.key === 'Enter' || e.key === ' ') && (e.preventDefault(), sortContacts('distance'))}
                  title={m.kad_col_distance_help()}
                >
    ...
    >
      <div class="modal-content bootstrap-modal">
        ...
        <div class="modal-body">
          <!--
            Full tablist keyboard contract, not just the roles: Left/Right (and
            Home/End) move between tabs, only the selected tab is in the Tab
            order, and each tab points at the panel it controls. Announcing
            "tab, 1 of 3" while arrow keys did nothing was the worst of both
            worlds — a screen-reader user was told how to drive a control that
            only responded to Tab-then-Enter.
          -->
          <div class="bootstrap-tabs" role="tablist">
            {#each bootstrapModes as mode (mode)}
              <button
                type="button"
                role="tab"
                id={`kad-bs-tab-${mode}`}
                aria-selected={bootstrapMode === mode}
                aria-controls="kad-bs-panel"
                tabindex={bootstrapMode === mode ? 0 : -1}
                class:active={bootstrapMode === mode}
                onclick={() => (bootstrapMode = mode)}
                onkeydown={bootstrapTabsKeydown}
              >{bootstrapModeLabel(mode)}</button>
            {/each}
          </div>
          ...
          <div
            id="kad-bs-panel"
            role="tabpanel"
            aria-labelledby={`kad-bs-tab-${bootstrapMode}`}
            tabindex="-1"
          >
            {#if bootstrapMode === 'ip'}
...
<style>
  ...
  /* No `:last-of-type` reset here any more: the panel's last group lives in
     `NetworkStatusTiles`, which drops its own trailing rule. Stripping the
     border off the last group *this page* owns just deleted the separator
     between the KAD counters and the reachability tiles below them. */
