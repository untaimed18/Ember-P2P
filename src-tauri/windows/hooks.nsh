...
!macro NSIS_HOOK_POSTINSTALL
  ...
  ; Tauri passes /UPDATE for in-app NSIS updates and deliberately leaves
  ; desktop shortcuts alone in that mode. Recreating one here bypasses that
  ; safeguard and can make Windows show a second desktop icon. The install
  ; directory remains stable, so an existing shortcut's target and explicit
  ; icon reference stay valid across an update.
  ${If} $UpdateMode <> 1
