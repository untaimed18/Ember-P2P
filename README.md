...
- **Transfer Monitoring** — Real-time progress bars, per-source detail drawers, upload tracking, health indicators, and archive recovery. Downloads can be paused, resumed, stopped or cancelled in bulk (checkbox selection, or the Pause/Resume/Stop/Cancel All commands), and finished rows can be selected and removed from the list together.
...
- **Internationalization** — UI strings via Paraglide (`en`, `es`, `fr`, `pt-BR`, `de`, `zh-CN`, `it`, `ru`, `zh-TW`); see [docs/i18n.md](docs/i18n.md).
